...
class FirestoreItemKeyedDataSource(
    ...
    private val executor = FireStore.executor
    ...
    class Factory @JvmOverloads constructor(
        ...
        override fun create(): DataSource<DocumentSnapshot, DocumentSnapshot> =
            FirestoreItemKeyedDataSource(
                queryFactory.create(),
                queryFactory.createReverse(),
                errorCallback
            )
    }
    ...
    private fun addPageLoader(
        ...
        registrations.add(query.addSnapshotListener(executor, pageLoader))
    }

    private fun queryStartFromBeginning(loadSize: Long) = directQuery.limit(loadSize)

    private fun queryStartAt(document: DocumentSnapshot, loadSize: Long): Query {
        return directQuery.startAt(document).limit(loadSize)
    }

    private fun queryStartAfter(document: DocumentSnapshot, loadSize: Long) =
        directQuery.startAfter(document).limit(loadSize)

    private fun queryEndAt(document: DocumentSnapshot, loadSize: Long) =
        reverseQuery.startAt(document).limit(loadSize)

    private fun queryEndBefore(document: DocumentSnapshot, loadSize: Long) =
        reverseQuery.startAfter(document).limit(loadSize)
     ...
     */
    private fun LoadCallback<DocumentSnapshot>.reversedResults() =
        object : LoadCallback<DocumentSnapshot>() {
     ...
     */
    private val defaultErrorCallback = object :
        ErrorCallback {
