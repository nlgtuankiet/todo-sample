...
import com.sample.todo.dynamic.data.task.firestore.function.SearchResultDataSourcefactory
...
class TaskDataSourceImpl @Inject constructor(
    ...
    override suspend fun insert(entity: Task): Long = withContext(FireStore) {
    ...
    override suspend fun update(task: Task): Long = withContext(FireStore) {
        suspendCoroutine<Long> { continuation ->
            val data = task.toDocumentData()
    ...
    override fun getTaskMiniObservablePaged(pageSize: Int): Observable<PagedList<TaskMini>> {
        val queryFactory = com.sample.todo.dynamic.data.task.firestore.QueryFactory.Builder(taskCollectionRef)
    ...
    override suspend fun deleteTask(id: String): Long = withContext(FireStore) {
        suspendCoroutine<Long> { continuation ->
